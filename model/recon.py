...
class face_model:
    def __init__(self, args, model_path=None, net_path=None, mbnet_path=None):

        if model_path is None:
            model_path = "./assets/face_model.npy"
        ...
        if net_path is None:
            net_path = "./assets/net_recon.pth"
        if mbnet_path is None:
            mbnet_path = "./assets/net_recon_mbnet.pth"

        if args.backbone == 'resnet50':
            self.net_recon = networks.define_net_recon(
                net_recon='resnet50', use_last_fc=False, init_path=None
            )
            self.net_recon.load_state_dict(torch.load(net_path)['net_recon'])
        ...
        if args.backbone == 'mbnetv3':
            ...
            self.net_recon.load_state_dict(torch.load(mbnet_path)['net_recon'])
            self.net_recon = self.net_recon.to(self.device)
            self.net_recon.eval()
